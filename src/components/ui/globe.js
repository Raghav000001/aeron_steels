...
function Globe({ globeConfig, data }) {
  ...
  useEffect(() => {
    ...
    globeRef.current
      ...
      .arcStroke(() => [0.4, 0.35, 0.38][Math.round(Math.random() * 2)])
    ...
    globeRef.current
      ...
      .ringMaxRadius(defaultProps.maxRings * 1.2)
...
export function World(props) {
  ...
  return (
    <Canvas scene={scene} camera={{ fov: 50, near: 180, far: 1800 }}>
      <WebGLRendererConfig />
      <ambientLight color={globeConfig.ambientLight} intensity={0.15} />
      <directionalLight
      ...
      <directionalLight
        ...
        position={new Vector3(250, 100, -400)}
        intensity={0.8}
      />
      <directionalLight
        color="#554455"
        position={new Vector3(400, -50, 200)}
        intensity={0.15}
      />
      <pointLight
        color={globeConfig.pointLight}
        position={new Vector3(-100, 400, 200)}
        intensity={0.4}
      />
      <Globe {...props} />
      <OrbitControls
